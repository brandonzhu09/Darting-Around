      ...
      <div class="absolute bg-white mt-2 max-h-32 max-w-xxs overflow-auto">
        {flightData["rows"]
          .filter(
            (flight) =>
              !clicked &&
              (flight.name.toLowerCase().includes(location.toLowerCase()) ||
                flight.iata.toLowerCase().includes(location.toLowerCase()))
          )
          .map((flight) => (
            <div
              class="block py-2 pl-2 text-xs hover:bg-gray-300"
              value={flight.name}
              onClick={() => {
                setLocation(flight.name);
                setLocationCode(flight.iata);
                setClicked(true);
              }}
            >
              {flight.name}
