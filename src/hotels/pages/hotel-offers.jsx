...
export default function HotelOffers() {
  ...
  const rooms =
    searchParams.get("rooms") === "" ? 1 : searchParams.get("rooms");

  const hotelImage =
    "https://mobileimg.pclncdn.com/htlimg/master/48/5/1/4851305/master_4851305";
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:5000/hotels/${location}/${departureDate}/${returnDate}`
      )
      .then((response) => {
          ...
          <div class="flex flex-wrap items-stretch">
            {hotels.map((hotel) => (
              <HotelCard
                ...
                amenities={
                  hotel.hotelFeatures.hotelAmenityCodes
                    ? hotel.hotelFeatures.hotelAmenityCodes
                        .slice(0, 2)
                        .map((hotelCode) =>
                          hotelCode in amenitiesCode
                            ? amenitiesCode[hotelCode]
                            : ""
                        )
                    : []
                }
